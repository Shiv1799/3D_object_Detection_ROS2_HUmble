...
class PCDetROS(Node):
    ...
    def __subsample_points__(self, points: np.ndarray) -> np.ndarray:
        ...
        if self.__max_subsample_points__ is not None and len(points) > self.__max_subsample_points__:
    ...
    def __runTorch__(self, points):
        ...
        # Deterministic padding using cyclic duplication
        if len(points) < self.__num_points_required__:
            repeats = (self.__num_points_required__ // len(points)) + 1
            points = np.tile(points, (repeats, 1))[:self.__num_points_required__]

        self.__points__ = points.reshape([-1, self.__num_features__])
        self.__points__ = self.__subsample_points__(self.__points__)
        ...
        with torch.no_grad():
            try:
                ...
                pad = self.__num_points_required__ - input_dict['points'].shape[0]
                if pad > 0:
                    # Deterministic padding for exception case
                    repeats = (pad // input_dict['points'].shape[0]) + 1
                    dup_points = np.tile(input_dict['points'], (repeats, 1))[:pad]
                    input_dict['points'] = np.concatenate((input_dict['points'], dup_points), axis=0)
                data_dict = self.__online_detection__.prepare_data(data_dict=input_dict)
    ...
    def __readConfig__(self):
        ...
        self.__num_points_required__ = 0
        self.__max_subsample_points__ = None
        if hasattr(cfg.DATA_CONFIG, 'DATA_PROCESSOR'):
            for proc in cfg.DATA_CONFIG.DATA_PROCESSOR:
                if proc['NAME'] == 'sample_points':
                    self.__num_points_required__ = proc['NUM_POINTS'].get('test', 0)
                    self.__max_subsample_points__ = self.__num_points_required__ * 5 if self.__num_points_required__ > 0 else None
                    break
    ...
    def __initParams__(self):
        ...
        self.declare_parameter("use_pseudo_lidar", rclpy.Parameter.Type.BOOL)
        self.declare_parameter("vertical_resolution_deg", rclpy.Parameter.Type.DOUBLE)
        self.declare_parameter("horizontal_resolution_deg", rclpy.Parameter.Type.DOUBLE)
        ...
        self.__use_pseudo_lidar__ = self.get_parameter("use_pseudo_lidar").value
        self.__v_res_deg__ = self.get_parameter("vertical_resolution_deg").value
        self.__h_res_deg__ = self.get_parameter("horizontal_resolution_deg").value
